...
При виклику функцій процесор використовує стек для збереження адреси повернення (Instruction Pointer, IP / RIP в x86-64). Це дозволяє повернутися до правильного місця виконання після завершення функції. У цьому завданні ми аналізуємо, чи можна обійтися без лічильника команд (IP), використовуючи тільки стек.

### Код з використання IP
...
```
При виклику `foo()` процесор зберіга IP (адресу наступної інструкції після `foo()`) у стеку. Після завершення `foo(`), процесор бере адресу повернення зі стека та продовжує виконання `main()`.

Код працює коректно, оскільки використовується стандартний механізм повернення.

### Результат виконання програми

![](task5/task5_IP.png)

### Код без використання IP
...
```
`func()` викликається, процесор зберігає IP у стеку (адресу, куди треба повернутися після `func()`). Функція`hack_return()` змінює стек, зсуваючи `rsp` (Stack Pointer), таким чином видаляючи адресу повернення. Коли `func()` завершується, процесор не може отримати правильний IP, що призводить до непередбачуваної поведінки процесу. Програма не повертається в `main()` і, ймовірно, завершиться з сегментаційною помилкою (Segmentation Fault) або непередбаченим кодом.
...
Відповідь: ні, IP потрібен для корекнтого виконання програми.
Причини:
1. Процесор використовує IP для читання наступної інструкції. Якщо IP не змінюється не не вказує на правильний код, то виконання програми буде некоректним.
2. IP змінюється атвоматично при кожному виконанні інструкції. Неможливо записати нову адресу в стек без сторонніх маніпуляцій.
3. Використання лише стеку нестабільне. Маніпуляції, подібні до `hack_return()`, можуть призвести до аварійного завершення небезпечних експлойтів.

# Завдання 6 (варіант 20)

## Умова

20. Реалізуйте стековий аналізатор відстеження викликів.
...
Ця програма реалізує стековий аналізатор викликів, який дозволяє відстежувати, які функції були викликані до поточної точки виконання. Це корисно для налагодження та розуміння послідовності викликів у програмі.

1. Виклик функцій від `main()` до `function3()`
   - Програма починає виконання з `main()`, яка викликає `function1()`.
   - `function1()` викликає `function2()`, а `function2()` – `function3()`.
   - У `function3()` викликається спеціальна функція `print_stack_trace()`, яка друкує стек викликів.
2. Як відбувається аналіз стеку?
   - Коли виконується `print_stack_trace()`, програма отримує список усіх функцій, які були викликані до цього моменту.
   - Для цього використовується `backtrace()`, яка зчитує адреси повернення функцій у стеку.
   - Потім `backtrace_symbols()` конвертує ці адреси в імена функцій, які можна прочитати у вигляді тексту.
   - Виведений список показує послідовність викликів, починаючи з `function3()` і закінчуючи `main()`.
3. Що ми бачимо у виводі?
   - Вивід програми показує стек викликів, тобто які функції викликали одна одну.
   - Це допомагає зрозуміти, як відбувається виконання програми і в якому порядку відбувалися виклики.
